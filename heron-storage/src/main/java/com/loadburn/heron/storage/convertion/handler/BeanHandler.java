...
public class BeanHandler<T> implements ResultHandler<T> {
    ...
    @Override
    public T handle(final EntityMetadata.EntityDescriptor entityDescriptor, ResultSet rs) throws SQLException {
        return rs.next() ? ResultSetConverter.delegation(entityDescriptor).toBean(rs, this.type) : null;
    }
